...
class EditPromptPage extends StatefulWidget {
  ...
  const EditPromptPage({Key? key, required this.prompt, required this.onEditPrompt}) : super(key: key);
...
class _EditPromptPageState extends State<EditPromptPage> {
  ...
  void initState() {
    ...
    _categoryController = TextEditingController(text: widget.prompt.category ?? 'other');
    _languageController = TextEditingController(text: widget.prompt.language ?? '');
  }
  ...
  void onSavePrompt() async {
    ...
    try {
      await PromptRepo.instant.updatePrompt(
  ...
  void onDeletePrompt() async {
    ...
    try {
      await PromptRepo.instant.deletePrompt(promptId: widget.prompt.id ?? '');
      widget.onEditPrompt();
      if (mounted) Navigator.pop(context);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Prompt deleted!'), backgroundColor: Colors.green),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: 'Delete',
            onPressed: isDeleting ? null : onDeletePrompt,
          ),
        ],
      ),
      body: Center(
                      ...
                      builder: (context, scale, child) {
                        return Transform.scale(
                          ...
                          child: ElevatedButton(
                            ...
                            onPressed: isLoading || isDeleting ? null : onSavePrompt,
                            child: isLoading
                                ? const SizedBox(
                                    width: 24,
                                    height: 24,
